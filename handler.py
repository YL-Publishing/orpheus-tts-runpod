...
def handler(event):
    ...
    SINGLE MODE:
        ...
        output: { "audio_base64": "...", "duration_s": 2.3, ... }

    BATCH MODE:
        input: {
            "segments": [
                { "id": "N1", "text": "First line", "voice": "tara" },
                { "id": "N2", "text": "Second line", "voice": "leo" },
            ]
        }
        output: {
            "results": [
                { "id": "N1", "audio_base64": "...", "duration_s": 2.3 },
                { "id": "N2", "audio_base64": "...", "duration_s": 1.8 },
            ]
        }
    """
    inp = event["input"]
    ...
    if "segments" in inp:
        ...
        for seg in segments:
            ...
            try:
                ...
                # Calculate duration from WAV size
                # WAV: 44 byte header, 16-bit mono @ 24kHz = 48000 bytes/sec
        ...
        print(f"Batch complete: {len(results)} segments in {total_time:.1f}s")
